...
class My_model extends CI_Model{
	...
	function get_customer() {
        ...
        $query = $this->db->query("select * from `vg_users` where `UserType`='2'");    
...

    {
        ...
        $query = $this->db->query("select a.ID,a.UserName,a.UserPhone,a.UserEmail,a.UserProfile,a.`UserType`,b.`Skills`,b.`experience`,b.`cost_hours`,b.`address` as traingAddress,b.`availableHoursStart`,b.`availableHoursEnd`,b.`availableMon`,b.`availableThe`,b.`availableWed`,b.`availableThu`,b.`availableFri`,b.`availableSat`,b.`availableSun` from `vg_users` as a join `vg_trainerProfile` as b on (a.`ID`=b.`userID`) where `UserType`='1'");
...
//==========================================================LOG Maintains========================
    ...
    {
        $query = $this->db->query("SELECT wp.`id`,usr.`UserName`,wp.`day` FROM `vg_workout_plan` as wp join `vg_users` as usr on wp.`trainee_id` = usr.`ID` where usr.`UserType`=2");    
